...
(function($) {
    ...
    $.fn.markyourcalendar = function(opts) {
        ...
        `;
        var nextHtml = `<div id="myc-next-week"><i class="fa fa-chevron-right"></i></div>`;
        var defaults = {
        ...
        this.getNavControl = function() {
			// stop the past day / yesterday selection
			// modify start
			let today = new Date();
			if(parseInt(settings.startDate.getTime()/1000) <= parseInt(today.getTime()/1000))
				settings.prevHtml = '';
			else
        ...
        this.getDatesHeader = function() {
            ...
            for (i = 0; i < 7; i++) {
                ...
                tmp += `
                    <div class="myc-date-header" id="myc-date-header-` + i + `" data-date="` + d.getTime() + `">
                        <div class="myc-date">` + settings.weekdays[d.getDay()] + `</div>
                        <div class="myc-date">` + d.getDate() + `</div>
                        <div class="myc-date">` + monthYearHtml + `</div>
                    </div>
                `;
            }
        ...
        this.getAvailableTimes = function() {
            ...
            for (i = 0; i < 7; i++) {
                ...
                $.each(settings.availability[i], function() {
                    var var_values = this.split('_ID_');
                    var timehere = var_values[0];
                    timehere = timehere.split('To');
					var tdate = formatDate(settings.startDate.addDays(i));
					var sd = tdate.split('-');
					var dbTime = sd[1] + '/' + sd[0] + '/' + sd[2] + ' ' + timehere[0] + settings.doctorZone;
					var localTime = new Date(typeof dbTime === "string" ? new Date(dbTime) : dbTime).toLocaleString("en-US", {timeStyle: 'short', timeZone: tzString});
					var dbtimems = new Date(dbTime).getTime();
					
					if(todayms < dbtimems){
						tmpAvailTimes += `
							<a href="javascript:;" class="myc-available-time" data-id="` + var_values[1] + `" data-time="` + var_values[0] + `" data-date="` + tdate + `">
								` + localTime + `
							</a>
						`;
					}
