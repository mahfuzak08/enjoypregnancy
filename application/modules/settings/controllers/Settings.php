...
class Settings extends MX_Controller {
	...
	function faq() {
        $data['settings'] = $this->settings_model->getSettings();
		if(isset($_GET['id'])){
			if($this->settings_model->deleteFaq($_GET['id'])){
	...
	function faq_update() {
		...
		if(isset($_GET['id']) && $_GET['id'] > 0)
			$data['faq'] = $this->settings_model->getFaqs($_GET['id']);
		if(isset($_POST['faq-submit'])){
			$vlink = str_replace(' ','_',$_FILES['vlink']['name']);
			$vlink = $this->uploadDoctorfiles($vlink,'vlink');
			$data = array(
				"title"=>$_POST["title"],
			...
			if($this->settings_model->setFaq($data) > 0){
				$this->session->set_flashdata('feedback', 'FAQ add/ update successfully.');
				redirect('settings/faq');
			}else{
				$this->session->set_flashdata('feedback', 'Update error');
			}
		}
	...
	function manage_api() {
		if(isset($_POST["api-submit"])){
			if($this->settings_model->set_api_info($_POST))
				redirect('settings/manage_api?t='.$_POST['type']);
			else
				exit();
		}
        if($_GET['t'] == 'call'){
			$data['head_label'] = "Calling Gateway";
			$data['labels'] = array("Meeting Domain", "External API URL");
		}
        elseif($_GET['t'] == 'chat'){
			$data['head_label'] = "Chatting Server";
			$data['labels'] = array("Socket Server URL");
		}
        elseif($_GET['t'] == 'squareup'){
			$data['head_label'] = "Square Payment Gateway";
			$data['labels'] = array("Square web payment SDK", "Application ID", "Location ID", "POST URL", "Square-Version", "KEY");
		}
		elseif($_GET['t'] == 'stripe'){
			$data['head_label'] = "Stripe Payment Gateway";
			$data['labels'] = array("Public Key", "Secret Key");
		}
		elseif($_GET['t'] == 'sms'){
			$data['head_label'] = "SMS Gateway";
			$data['labels'] = array("Name", "SID", "Token", "Sender Number");
		}
		elseif($_GET['t'] == 'firebase'){
			$data['head_label'] = "Firebase API Details";
			$data['labels'] = array("API Key", "App ID", "Auth Domain", "Project ID", "Database URL", "Messaging Sender ID" );
		}
		elseif($_GET['t'] == 'currency'){
			$data['head_label'] = "Currency Conversion API";
			$data['labels'] = array("Domain", "API Key");
		}
		elseif($_GET['t'] == 'giphy'){
			$data['head_label'] = "Giphy API";
			$data['labels'] = array("API Key");
		}
		
		$data['apiinfo'] = $this->settings_model->get_api_info($_GET['t'], 0);
        $this->load->view('home/dashboard', $data);
        $this->load->view('manage_api', $data);
        $this->load->view('home/footer');
    }
	...
	function uploadDoctorfiles($image,$value)
    {
        ...
        if ($this->upload->do_upload($value)) {
            ...
            return "assets/faq/" . $path1['file_name'];
        } else {
