...
class Pharmacy_model extends CI_model {
    ...
    function getproductOrders()
    {
        $this->db->order_by('id','DESC');
        $this->db->limit(100);
        return $this->db->get('mp_orders')->result();
    }
    ...
    function getlabOrders()
    {
        $this->db->order_by('id','DESC');
        $this->db->limit(100);
        return $this->db->get('mp_lab_orders')->result();
    }

    function getorderinformation($pid)
    {
        $this->db->where('id',$pid);
        return $this->db->get('mp_orders')->row_array();
    }

    function getBookedproductInfo($pid)
    {
        $this->db->where('id',$pid);
        return $this->db->get('medicine_another')->row_array();
    }

    function getproducAssignedtOrders()
    {
        if($this->ion_auth->in_group(array('Pharmacist')))
        {
            ...
            {
                $this->db->order_by('id','DESC');
                $this->db->where('assign_to_saleman', $user_id_here);
                return $this->db->get('mp_orders')->result();
            }
            else
            {
                ...
                $pharmacy_id = $this->db->get('users')->row()->hospital_ion_id;

                $this->db->order_by('id','DESC');
                $this->db->where('assign_to', $pharmacy_id);
                return $this->db->get('mp_orders')->result();
            }
        }
        else
    ...
    function forwardOrdertopharmacy($data)
    {
        $update_data = array('status' => 'in_progress', 'assign_to' => $data['pharmacy_id']);
        $this->db->where('id',$data['order_id']);
        $this->db->update('mp_orders', $update_data);
    }

    function markascompleteOrder($id)
    {
    ...
    function cancelProductorder($data)
    {
        $update_data = array('status' => 'cancelled','cancel_reason' => $data['reason']);
        $this->db->where('id', $data['order_id']);
        $this->db->update('mp_orders', $update_data);
    }
    ...
    function getsalemans()
    {
        ...
        $this->db->where('id',$uid);
        $pharmacy_id = $this->db->get('users')->row()->hospital_ion_id;
        $this->db->where(array('hospital_id'=>$pharmacy_id, 'is_saleman'=>1));
