...
class Pharmacy extends MX_Controller {
    ...
    public function porders() {
        ...
        $data['settings'] = $this->settings_model->getSettings();
        $data['orders'] = $this->pharmacy_model->getproductOrders();
        $data['pharmacies'] = $this->hospital_model->getPharmacies();
        // echo "<pre>";
        // print_r($data['orders']);
        // exit;
        $this->load->view('home/pharmacy-dashboard'); // just the header file
        $this->load->view('pharmacy/porders', $data);
    ...
    function forwardOrdertoPharmacy()
    {
        $data = $this->input->post();
        $this->pharmacy_model->forwardOrdertopharmacy($data);
        $this->session->set_flashdata('feedback','Order forwarded to pharmacy');
        redirect('hospital/pharmacy/porders');
    }
