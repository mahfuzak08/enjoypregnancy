...
$this->db->where('hospital_id', $this->session->userdata('hospital_id'));
    ...
    // haseen code here
    function insertTimeSlot1($data) {
        $data1 = array('hospital_id' => $this->session->userdata('hospital_id'));
        $data2 = array_merge($data, $data1);
        $this->db->insert('time_slot', $data2);
    }

    function insertSchedule1($data) {
        $data1 = array('hospital_id' => $this->session->userdata('hospital_id'));
        $data2 = array_merge($data, $data1);
        $this->db->insert('time_schedule', $data2);
    }

    function getscheduleddaybyDoctorId($doctorr)
    {
        $this->db->select('weekday');
        $this->db->where('doctor',$doctorr);
        return $this->db->get('time_schedule')->result();
    }

    function getSlotById($id)
    {
        $this->db->where('id', $id);
        return $this->db->get('time_slot')->row();
    }
	
	function getSlotByDoctorId($id)
    {
	...
	function getslotbyidasAjax($data)
    {
        $doctor_id = $data['doctor_id'];
        $weekday = $data['weekday'];  
        $this->db->where(array('doctor'=>$doctor_id,'weekday'=>$weekday));
        $this->db->order_by('id');
        return $this->db->get('time_slot')->result();     
    }

    function updateSchedule1($schedule_id,$data)
    {
        $this->db->where('id',$schedule_id);
        $this->db->update('time_schedule', $data);
    }

    function deleteexistingtimeSlots($doctor,$weekday)
    {
        $this->db->where(array('doctor'=>$doctor,'weekday'=>$weekday));
