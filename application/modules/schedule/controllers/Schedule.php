...
class Schedule extends MX_Controller {
	...
	function deleteSlot() {
        $schedule_details = $this->schedule_model->getSlotById($_POST['id']);
        $res = $this->schedule_model->deleteTimeSlotByDoctorByWeekday($schedule_details->doctor, $schedule_details->weekday);
        echo $res;
    }
	...
	function addSchedule2(){
        ...
        $duration = str_replace(" Hour", "", $duration);
        ...
        if (empty($s_time)) {
            ...
            redirect('doctor/schedule_timing');
            die();
        }
		...
		// if($id > 0){
			// $this->schedule_model->deleteexistingtimeSlots($doctor,$weekday);
			// $this->schedule_model->updateSchedule1($schedule_id,$data);
		// }
        for($i=0; $i<count($s_time); $i++)
        {
            ...
            $this->schedule_model->insertTimeSlot1($slot_data);
        }
        $this->schedule_model->insertSchedule1($data);
    ...
    function geteditslotsbyAjax()
    {
        $res = $this->schedule_model->getslotbyidasAjax($_GET);
        echo json_encode($res);
    }

    function editSchedule1()
    {
        $redirect = $this->input->post('redirect');
        if (empty($redirect)) {
            $redirect = 'schedule';
        ...
        $duration = $this->input->post('duration');
        $s_time_key = $this->input->post('s_time_key');
        // $schedule_id = $this->input->post('id');
        // echo "<pre>";
...
// Validating Email Field
	...
	function addHoliday2() {
        $this->form_validation->set_rules('doctor_id', 'Loading problem', 'trim|required');
        $this->form_validation->set_rules('s_date', 'Start Date', 'trim|required');
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			$date = $this->input->post('s_date').' - '.$this->input->post('e_date');
	...
	function deleteTimeSlot2() {
        $id = $this->input->post('id');
        if($this->schedule_model->deleteTimeSlot($id))
			echo 'Done';
		else 
			echo 'Error';
    }
