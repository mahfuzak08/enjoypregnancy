...
class Meeting extends MX_Controller {
    ...
    function liveChatApp() { //jitsiLive
        $data['roomId'] = $this->input->get('roomId');
        $call_type = $this->input->get('type');
        // $data['sender'] = $this->input->get('sender');
        $data['sender'] = $this->ion_auth->get_user_id();
		$data['room_info'] = $this->chat_model->getOneConversation($data['roomId'], $data['sender'], $call_type)[0];
		$data['participants'] = $this->chat_model->getParticipantsInfo($data['room_info']->participants_ids);
        
		$this->sendPushNotification2Participants($data);
        
		if( $this->input->get('m') == 2) $this->load->view('home/css'); else  $this->load->view('home/dashboard'); // just the header file
        $this->load->view('jitsichatforpatient', $data);
        //$this->load->view('home/footer'); // just the header file
    }
	
	function sendPushNotification2Participants($data)
    {
        //   echo $call_type.'<br>'.$roomid.'<br>'.$p_id.'<br>'.$dr_id; exit;
        $title = "Incomming Call";
        $type_h = "incomming_call";
		
		foreach($data['participants'] as $row){
			if($row->id == $data['sender']){
				$sender_name = $row->username;
				$sender_img = base_url().$row->img;
			}
		}
		
		//FCM API end-point
		$url = 'https://fcm.googleapis.com/fcm/send';
		$server_key = 'AAAAlgUaefM:APA91bFiD5Cn5g5S6dvM0DVi7tqsyZ1gNxLKt-dUhAZmYv29_MOLCVBlvCf-2ATFf9eRtmto3qA-afaWXFctwufxc9K-hjHPG2YmZk2_yUtR_ssqI1PxP7qaWJLO8AFr3Dpx79avHQju';
		...
		foreach($data['participants'] as $row){
			if($row->id == $data['sender']) continue;
			...
			$json_data = array (
				...
				"data" => array (
					...
					"body" => "New Incomming call from ".$sender_name,
					"doc_id" => $data['sender'],
					"doc_name" => $sender_name,
					"extraData" => $sender_img,
					"title" => 'Incoming Call',
					"type" => 'incomming_call',
					"room" => $data['roomId'],
				),
    ...
    function liveChatAdminDoctorApp() { //jitsiLive
        $roomId = $this->input->get('roomId');
        $call_type = $this->input->get('type');
        $u_ids = explode('-',$roomId);

        $this->sendPushNotificationAdminDoctor($call_type, $roomId, $u_ids[0], $u_ids[1]);
        // echo $roomId; exit;
        $data['roomId'] = $roomId;

        $data['user_level1'] = 'Admin';
        $data['user_level2'] = 'Doctor';

        $data['user1_details'] =$user1_details= $this->profile_model->getProfileById($u_ids[1]);
       // print_r($data['user1_details']); exit;
       $data['user2_details']= $user2_details = $this->doctor_model->getDoctorById($u_ids[0]);

        $data['user1_name'] = $user1_details->username;
        $data['user2_name'] = $user2_details->name;
        if( $this->input->get('m') == 2) $this->load->view('home/css'); else  $this->load->view('home/dashboard'); // just the header file
        $this->load->view('jitsichatforpatient', $data);
        //$this->load->view('home/footer'); // just the header file
    }
    ...
    function jitsi() {
        ...
        redirect('meeting/liveApp?id=' . $appointment_id);
    }
...
// API Requests Ends
    ...
    function sendPushNotification($call_type, $roomid, $p_id, $dr_id)
    {
        //   echo $call_type.'<br>'.$roomid.'<br>'.$p_id.'<br>'.$dr_id; exit;
        if($call_type==1)
        {
        ...
        {
            $title = "Incomming Voice Call";
            $type_h = "incomming_call_voice";
        }
        $doctor_details = $this->doctor_model->getDoctorById($dr_id);
        $doctor_name = $doctor_details->name;
        $doctor_img_url = base_url().$doctor_details->img_url;
        // {"data":{"body":"New Incomming call from Mr Doctor","extraData":"https://callgpnow.com/public/uploads/21.JPG","room":"62-162","title":"Incoming Call","type":"incomming_call"},"to":"/topics/p62"}
        $json_data = array (
            "to" => "/topics/p".$p_id,
            "priority" => "high",
            // "notification" => array (
            //     "title" => $title,
            //     "body" => "Calling from ".$doctor_name
            // ),
            "data" => array (
                // "click_action" => "FLUTTER_NOTIFICATION_CLICK",
                "body" => "New Incomming call from ".$doctor_name,
                "doc_id" => $dr_id,
                "doc_name" => $doctor_name,
                "extraData" => $doctor_img_url,
                "title" => 'Incoming Call',
                "type" => 'incomming_call',
                "room" => $roomid,
            ),
    ...
    function sendPushNotificationAdminDoctor($call_type, $roomid, $dr_id, $admin_id)
    {
    //   echo $call_type.'<br>'.$roomid.'<br>'.$p_id.'<br>'.$dr_id; exit;
        if($call_type==1)
        {
        ...
        $user_details = $this->profile_model->getProfileById($admin_id);
        $user_name = $user_details->name;
        $user_img_url = base_url().$user_details->img_url;
        $json_data = array (
            "to" => "/topics/p".$dr_id,
            "priority" => "high",
            "notification" => array (
                    "title" => $title,
                    "body" => "Calling from ".$user_name
            ),
            "data" => array (
                    "click_action" => "FLUTTER_NOTIFICATION_CLICK",
                    "doc_id" => $dr_id,
                    "doc_name" => $user_name,
                    "doc_photo" => $user_img_url,
                    "type" => $type_h,
                    "room" => $roomid,
            ),
        );
        ...
        $url = 'https://fcm.googleapis.com/fcm/send';
        //api_key in Firebase Console -> Project Settings -> CLOUD MESSAGING -> Server key
        $server_key = 'AAAA0EpRwPY:APA91bHBbBup11jcpJ65yZKqUqkUK5IPDUN9O51ade_qcoFKZdqyUuiK07v3mFSUmrA2ZAEP1M0zV09a794SZPOlmvbvDAOHN5cNdKNst0aCMq4WJIKbhDMWPK0ks-obO7rUd_vgTGIn';
        //header with content_type api key
        $headers = array(
            'Content-Type:application/json',
            'Authorization:key='.$server_key
