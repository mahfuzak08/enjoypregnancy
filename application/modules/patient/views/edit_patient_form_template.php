    ...
    <section class="wrapper site-min-height">
        ...
        <section class="">
            ...
            <div class="panel-body">
                <form action="<?= base_url('patient/update_patient_template?id=' . $template->id) ?>" method="post">
                    <div class="adv-tableX  ">

                        <div class="space15"></div>
                        <div class="form-group">
                            <label>Tempalte Name</label>
                            <input class="form-control" name="template" value="<?= $template->name; ?>">
                        </div>
                        <div class="col-md-7">
                                ...
                                foreach ($data->section as $key => $section) { ?>
                                    <div class="sectionPanel">
                                        <div class="panel panel-info "
                                             style="border: 1px solid #dedede; padding-top: 0;">
                                            <div class="panel-heading" style="margin-top: 0;">
                                                <div class="row">
                                                    <div class="col-md-8"> Section title <input type="text"
                                                                                                class="form-control"
                                                    ...
                                                    <div class="col-md-4 text-right">
                                                        <button type="button"
                                                                class="btn btn-danger btn-flat deletebtn  "
                                                                onclick="this.parentNode.parentNode.parentNode.parentNode.remove(this.parentNode);">
                                                            Remove Section
                                                        </button>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="QuizArea">
                                                ...
                                                if (!empty($section->quiz))
                                                    foreach ($section->quiz as $index => $quiz) { ?>
                                                        <div class="panel-body sectionQuestion">
                                                            <button type="button"
                                                                    class="btn btn-danger btn-flat deletebtn pull-right "
                                                                    onclick="this.parentNode.remove(this.parentNode);">
                                                                Remove question
                                                            </button>
                                                            <div class="form-group "><label>Question title </label>
                                                                <input type="text" class="form-control"
                                                                       name="section[<?= $key ?>][quiz][<?= $index ?>][title]"
                                                                       value="<?= $quiz->title ?>"></div>
                                                            <div class="form-group "><label>Choose Type : </label>
                                                                <select name="section[<?= $key ?>][quiz][<?= $index ?>][quize_type]"
                                                                        class="form-control quize_type"
                                                                        data-index="<?= $index ?>"
                                                                        data-section_index="<?= $key ?>">
                                                                    <option value="SLT" <?= ($quiz->quize_type == 'SLT') ? 'selected' : '' ?>>
                                                                        Single Line text
                                                                    </option>
                                                                    <option value="SC" <?= ($quiz->quize_type == 'SC') ? 'selected' : '' ?>>
                                                                        Single Choice
                                                                    </option>
                                                                    <option value="MC" <?= ($quiz->quize_type == 'MC') ? 'selected' : '' ?>>
                                                                        Multiple Choice
                                                                    </option>
                                                                    <option value="PF" <?= ($quiz->quize_type == 'PF') ? 'selected' : '' ?>>
                                                                        Paragraph Field
                                                                    </option>
                                                                    <option value="Signature" <?= ($quiz->quize_type == 'Signature') ? 'selected' : '' ?>>
                                                                        Signature
                                                                    </option>
                                                                    <option value="Attach" <?= ($quiz->quize_type == 'Attach') ? 'selected' : '' ?>>
                                                                        Attachment
                                                                    </option>
                                                                </select>
                                                            </div>
                                                            <div class="form-group"><label> Required? </label>
                                                                <input type="checkbox"
                                                                       name="section[<?= $key ?>][quiz][<?= $index ?>][required]"
                                                                       value="1" <?= ($quiz->required == 1) ? 'checked' : '' ?>>
                                                            </div>

                                                            <div class="SectionAnsOptions form-group">
                                                                <div class="border-bottom"></div>
                                                                <div class="col-md-10">
                                                                    <?php
                                                                    if ($quiz->quize_type == 'SC')
                                                                        foreach ($quiz->ans as $ans) { ?>
                                                                            <div class="input-group"><span
                                                                                        class="input-group-addon">  <input
                                                                                            type="radio">  </span>
                                                                                <input type="text" class="form-control"
                                                                                       name="section[<?= $key ?>][quiz][<?= $index ?>][ans][]"
                                                                                       value="<?= $ans ?>"> <span
                                                                                        class="input-group-btn"><button
                                                                                            type="button"
                                                                                            class="btn btn-danger btn-flat deletebtn  "
                                                                                            onclick="this.parentNode.parentNode.remove(this.parentNode);"><i
                                                                                                class="fa fa-trash"></i></button> </span>
                                                                            </div>
                                                                        <?php }

                                                                    if ($quiz->quize_type == 'MC')
                                                                        foreach ($quiz->ans as $qf => $ans) { ?>
                                                                            <div class="input-group"><span
                                                                                        class="input-group-addon">  <input
                                                                                            type="checkbox">  </span>
                                                                                <input type="text" class="form-control"
                                                                ...
                                                                <?php if ($quiz->quize_type == 'MC' || $quiz->quize_type == 'SC') { ?>
                                                                    <div>
                                                                        <button type="button"
                                                                                class="btn  btn-default addAnsBtn"
                                                                                data-optiontype="<?= $quiz->quize_type; ?>"
                                                                                data-section_index="<?= $key ?>"
                                                                                data-quiz_field="<?= $index; ?>">Add answer
                                                                        </button>
                                                                    </div>
                                                                <?php } ?>
                                            ...
                                            <div class="panel-footer">
                                                <button type="button" class="btn btn-default addQuizBtn"
                                                        data-section_index="<?= $key ?>">Add Question
                                                </button>
                                            </div>
                                        </div>
