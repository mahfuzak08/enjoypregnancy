...
class Prescription extends MX_Controller {
  ...
  public function addbodychart() {
        ...
        $this->load->view('prescription/addbodychart_view', $data);
        $this->load->view('home/footer'); // just the header file
    }

    public function bodycharttemplate() {
        $data = array();
        $data['settings'] = $this->settings_model->getSettings();
    ...
    public function editBodyTemplateByJason() {
        $id = $this->input->get('id');
        $data['template'] = $this->bodycharttemplate_model->getBodychartById($id);
        echo json_encode($data);
    }

    public function addNewBodyTemplate() {
        ...
        $this->form_validation->set_rules('title', 'Title', 'trim|required|min_length[1]|max_length[100]|xss_clean');
            ...
            if ($this->upload->do_upload('img_url')) {
                ...
                $img_url = "bodychart/" . $path['file_name'];
                $data = array(
                    'thumbnil' =>   $img_url,
                    'image' => $img_url,
                    'title' => $title
            ...
            if (empty($id)) {     // Adding New Doctor
                    ...
                    $dfg = 4;
                    $this->bodycharttemplate_model->insertBodychart($data);
                    $this->session->set_flashdata('feedback', lang('added'));

            } else { // Updating Doctor

                $this->bodycharttemplate_model->updateBodychart($id, $data);
                $this->session->set_flashdata('feedback', lang('updated'));
            }
            // Loading View
            // redirect('prescription/bodycharttemplate');
            redirect('doctor/bodycharttemplate');
    ...
    function Templatedelete() {
        ...
        $id = $this->input->get('id');
        $data = $this->db->get_where('bodycharttemplate', array('id' => $id))->row();
        $path = $data->image;
        ...
        $this->bodycharttemplate_model->delete($id);
    ...
    $limit = $requestData['length'];
        ...
        if ($limit == -1) {
            if (!empty($search)) {
                $data['templates'] = $this->bodycharttemplate_model->getBodychartBysearch($search);
            } else {
                $data['templates'] = $this->bodycharttemplate_model->getBodychart();
            }
        } else {
            if (!empty($search)) {
                $data['templates'] = $this->bodycharttemplate_model->getBodychartByLimitBySearch($limit, $start, $search);
            } else {
                $data['templates'] = $this->bodycharttemplate_model->getBodychartByLimit($limit, $start);
            }
        }
        //  $data['doctors'] = $this->doctor_model->getDoctor();
        ...
        foreach ($data['templates'] as $template) {
            if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist'))) {
                $options1 = '<a type="button" class="btn btn-info btn-xs btn_width editbutton" title="' . lang('edit') . '" data-toggle="modal" data-id="' . $template->id . '"><i class="fa fa-edit"> </i> ' . lang('edit') . '</a>';
             ...
             if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist'))) {
                $options3 = '<a class="btn btn-info btn-xs btn_width delete_button" title="' . lang('delete') . '" href="bodycharttemplate/delete?id=' . $template->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"> </i> ' . lang('delete') . '</a>';
            }
 ...
 public function bodychartview()
    {
        ...
        $id = $this->input->get('id');
        $data['template'] = $this->bodycharttemplate_model->getBodychartTemplateById($id);
        $data['template_pic'] = $this->bodycharttemplate_model->getBodychartTemplatePic($id);


        $data['settings'] = $this->settings_model->getSettings();
        $this->load->view('home/css'); // just the header file
        $this->load->view('prescription/bodychartview_view', $data);
        //$this->load->view('home/footer'); // just the header file
    }


   public function bodycharteditor()
    {
        if (!$this->ion_auth->in_group(array('admin', 'Doctor'))) {
            redirect('home/permission');
        }
         $template_id = $this->input->get('template_id');
         $data['template'] = $this->bodycharttemplate_model->getBodychartById($template_id);

        $data['settings'] = $this->settings_model->getSettings();
        $this->load->view('home/css'); // just the header file
        $this->load->view('prescription/bodycharteditor_view', $data);
        //$this->load->view('home/footer'); // just the header file
    }
    ...
    public function addPrescriptionView() {
       ...
       if($this->input->get('type') == 1) $this->load->view('home/css'); else $this->load->view('home/dashboard', $data); // just the header file
    ...
    function viewPrescription() {
        ...
        if (!empty($data['prescription']->hospital_id)) {
            ...
            if ($data['prescription']->hospital_id != $this->session->userdata('hospital_id')) {
                ...
                if($this->input->get('type') == 1) $this->load->view('home/css', $data);
               else  $this->load->view('home/dashboard', $data); // just the header file
