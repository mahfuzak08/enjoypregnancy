...
class Appointment extends MX_Controller {
    ...
    public function index() {
        ...
        if ($this->ion_auth->in_group(array('Doctor'))) {
            ...
            $data['urgent_appointment_requests'] = $this->doctor_model->getUrgentAppointmentsByDoctor($doctor);
        } else {
            $data['urgent_appointment_requests'] = $this->doctor_model->getUrgentAppointments();
        }
        if ($this->ion_auth->in_group(array('Doctor'))) {
            $doctor_ion_id = $this->ion_auth->get_user_id();
            $doctor = $this->db->get_where('doctor', array('ion_user_id' => $doctor_ion_id))->row()->id;
            $data['home_appointment_requests'] = $this->doctor_model->getHomeAppointmentsByDoctor($doctor);
        } else {
            $data['home_appointment_requests'] = $this->doctor_model->getHomeAppointments();
        }
    ...
    public function addNewApp() {
        ...
        if (empty($id)) {     // Adding New department
            ...
            $doctor_name = $this->appointment_model->getDoctorname($doctor);
            $this->session->set_flashdata('normal_app_feedback', '<h3>Appointment booked Successfully!</h3><p>Appointment booked with <strong>Dr. '.$doctor_name->name.'</strong><br> on <strong>'.date('d M, Y',$date).' '.$time_slot.'</strong></p>');
    ...
    function getAppointmentByJason() {
        ...
        foreach ($query as $entry) {
            ...
            $patient_mobile = $entry->patient_phone ? $entry->patient_phone : '';
            $patient_name = $entry->patient_name ? $entry->patient_name : '';
            $patient_dob = $entry->patient_dob ? $entry->patient_dob : '';
			
            $info = '<br/>' . lang('patient') . ': ' . $patient_name . '<br/>' . lang('phone') . ': ' . $patient_mobile . '<br/>DOB: ' . $patient_dob;
            $color = '#009988';
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            if (!empty($patientdetails)) {
               //$patientname = ' <a type="button" class="history" data-toggle = "modal" data-id="' . $appointment->patient . '"> ' . $patientdetails->name . '</a>';
                $patientname =  '<a type="button" href="javascript:;" class="historyBtn" data-pid="'.$appointment->patient.'">' . $patientdetails->name . '</a>';
        ...
        foreach ($data['appointments'] as $appointment) {
          ...
          /*  if (!empty($patientdetails)) {
            ...
            if (!empty($patientdetails)) {
               ...
               // $patientname = ' <a type="button" class="history" data-toggle = "modal" data-id="' . $appointment->patient . '"> ' . $appointment->patientname . '</a>';
                $patientname =  ' <a type="button" href="javascript:;" class="historyBtn" data-pid="'.$appointment->patient.'"> ' . $appointment->patientname . '</a>';
            }
            $doctordetails = $this->doctor_model->getDoctorById($appointment->doctor);
            if (!empty($doctordetails)) {
                $doctorname = $doctordetails->name;
    ...
    function getMyTodaysAppoinmentList() {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            if ($patient_id == $appointment->patient) {
                ...
                $doctordetails = $this->doctor_model->getDoctorById($appointment->doctor);
                if (!empty($doctordetails)) {
                    $doctorname = '<a href="frontend/doctor_profile/'.$appointment->doctor.'" target="_blank">'.$doctordetails->name.'</a>';
                } else {
                    $doctorname = $appointment->doctorname;
                }
    ...
    function homevisitappointments()
    {
        
        $data['homevisitAppointment'] = $this->appointment_model->gethomevisitAppointment();
        // echo "<pre>";
        // print_r($data['homevisitAppointment']);
        // exit;
        $data['patients'] = $this->patient_model->getPatient();
        $data['doctors'] = $this->doctor_model->getDoctor();
        $data['settings'] = $this->settings_model->getSettings();
        ...
        $this->load->view('home/dashboard', $data); // just the header file
        $this->load->view('homevisitappointment', $data);
