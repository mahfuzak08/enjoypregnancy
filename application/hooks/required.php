...
function required() {
    ...
    if ($RTR->class != "frontend" && $RTR->class != "pf" && $RTR->class != "request" && $RTR->class != "auth") {
    ...
    if ($RTR->class == "frontend" || $RTR->class == "request" ||  $RTR->class == "pf") {
        $CI->db->where('hospital_id', 'superadmin');
        $CI->language = $CI->db->get('settings')->row()->language;
        $CI->lang->load('system_syntax', $CI->language);
    }
    ...
    $common = array('auth', 'frontend', 'settings', 'home', 'patient', 'profile', 'request','pf', 'chat', 'payment');
    // echo $RTR->class."<br>";
