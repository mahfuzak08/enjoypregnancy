...
class Ion_auth_model extends CI_Model
{
	...
	{
	   // echo "<pre>"; print_r($additional_data['is_hospital']); exit;
		...
		$password   = $this->hash_password($password, $salt);
		if($additional_data['is_hospital']==1)
		{
			$is_hospital = 1;
		}
		elseif($additional_data['is_hospital']==2)
		{
			$is_hospital = 2;
		}
		else
...
// 	Haseen code
	public function register2($username, $password, $email, $dfg, $phone, $is_doc,  $additional_data = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
...
// end here
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=FALSE,$phonecode)
	{
...
// 		echo $identity;exit;
        ...
        $phone_num = $phonecode.$identity;
        $identity = $identity;
		$this->trigger_events('extra_where');
		$condition_w = "email='$identity' OR phone='$phone_num'";
		$query = $this->db->select('username, email, id, password, active, last_login, email_active, hospital_ion_id, is_hospital')
		                  ->where($condition_w)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
...
// 		exit;
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				{
					$this->set_error('login_email_inactive');
					return FALSE;
				}
				...
				if($user->email_active==2)
					$this->session->set_flashdata('feedback', 'Your email is not verified.');
	...
	public function loginViaOTP($identity, $remember=FALSE)
	{
		// echo 'ABC:'.$identity; exit;
		$this->trigger_events('pre_login');
...
// 		echo $identity;exit;
        ...
        $identity = $identity;
		$this->trigger_events('extra_where');
		$condition_w = "phone='$identity'";
		$query = $this->db->select('username, email, id, password, active, last_login, email_active, is_hospital')
		                  ->where($condition_w)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	**/
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'hospital_ion_id' => $user->hospital_ion_id
		);
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, username, email, last_login, is_hospital')
	...
	**/
	 ...
	 **/
	public function change_password_api($id, $new)
	{
